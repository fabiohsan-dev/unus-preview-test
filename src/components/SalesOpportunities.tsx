...
export function SalesOpportunities({ opportunities: initialOpportunities }: SalesOpportunitiesProps) {
  const displayOpportunities = initialOpportunities && initialOpportunities.length > 0
    ...
    <section id="investimentos" className="py-24 lg:py-32 px-6 sm:px-8 lg:px-12 bg-white">
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
          ...
          <p className="text-[var(--color-body)] text-[15px] leading-relaxed max-w-[360px]" style={{ fontWeight: 300 }}>
            Imóveis com potencial real de valorização, selecionados por dados de mercado e expertise local.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {displayOpportunities.map((item) => {
            const isTerrain = item.type.toLowerCase().includes('terreno') || item.type.toLowerCase().includes('lote');
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col flex-1">
                  <p className="text-[var(--secondary-400)] text-[11px] uppercase tracking-[0.12em] mb-2" style={{ fontWeight: 500 }}>
                    {item.location}
                  </p>
                  <h3 className="text-[var(--color-heading)] text-[22px] leading-tight mb-4 line-clamp-2 min-h-[52px]" style={{ fontWeight: 500 }}>
                    {item.title}
                  </h3>
                  
                  <div className="flex items-center gap-4 text-[var(--color-body)] text-[12px] mb-4 pb-4 border-b border-[var(--neutral-300)] flex-wrap" style={{ fontWeight: 400 }}>
                    {isTerrain ? (
                      <span className="flex items-center gap-1.5">
                        <Maximize className="w-4 h-4 text-[var(--secondary-400)]" strokeWidth={1.2} />
                        Área: {item.area}
                      </span>
                    ) : (
                      <>
                        <span className="flex items-center gap-1.5">
                          <BedDouble className="w-4 h-4 text-[var(--secondary-400)]" strokeWidth={1.2} />
                          {item.bedrooms} dorms.
                        </span>
                        <span className="flex items-center gap-1.5">
                          <Bath className="w-4 h-4 text-[var(--secondary-400)]" strokeWidth={1.2} />
                          {item.bathrooms} banh.
                        </span>
                        <span className="flex items-center gap-1.5">
                          <Maximize className="w-4 h-4 text-[var(--secondary-400)]" strokeWidth={1.2} />
                  ...
                  <p className="text-[var(--color-heading)] text-[20px] mt-auto" style={{ fontFamily: 'var(--font-serif)', fontWeight: 500 }}>
                    {item.price}
                  </p>
                </div>
              </Link>
